...
model {
  ...
  for (i in 1:N) {
    // Congruent at time 1
    RT[i,1,1,1:T_subj[i,1,1]] ~ lognormal(mu_i[i,1], sigma_i[i,1]);
    // Incongruent at time 1
    RT[i,2,1,1:T_subj[i,2,1]] ~ lognormal(mu_i[i,2], sigma_i[i,2]);
    // Congruent at time 2
    RT[i,1,2,1:T_subj[i,1,2]] ~ lognormal(mu_i[i,3], sigma_i[i,3]);
    // Incongruent at time 2
    RT[i,2,2,1:T_subj[i,2,2]] ~ lognormal(mu_i[i,4], sigma_i[i,4]);
  }
}
generated quantities { 
  ...
  for (i in 1:N) {
    ...
    for (t in 1:T_subj[i,1,1]) {
      post_pred_c1_t1[i,t] = lognormal_rng(mu_i[i,1], sigma_i[i,1]);
      log_lik[i,1,1,t] = lognormal_lpdf(RT[i,1,1,t] | mu_i[i,1], sigma_i[i,1]);
    }
    // Incongruent at time 1
    for (t in 1:T_subj[i,2,1]) {
      post_pred_c2_t1[i,t] = lognormal_rng(mu_i[i,2], sigma_i[i,2]);
      log_lik[i,2,1,t] = lognormal_lpdf(RT[i,2,1,t] | mu_i[i,2], sigma_i[i,2]);
    }
    // Congruent at time 2
    for (t in 1:T_subj[i,1,2]) {
      post_pred_c1_t2[i,t] = lognormal_rng(mu_i[i,3], sigma_i[i,3]);
      log_lik[i,1,2,t] = lognormal_lpdf(RT[i,1,2,t] | mu_i[i,3], sigma_i[i,3]);
    }
    // Incongruent at time 2
    for (t in 1:T_subj[i,2,2]) {
      post_pred_c2_t2[i,t] = lognormal_rng(mu_i[i,4], sigma_i[i,4]);
      log_lik[i,2,2,t] = lognormal_lpdf(RT[i,2,2,t] | mu_i[i,4], sigma_i[i,4]);
    }
  }
} 
