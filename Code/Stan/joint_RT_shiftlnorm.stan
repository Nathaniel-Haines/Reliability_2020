...
transformed parameters {
  ...
  for (i in 1:N) {
    ...
    ndt[i,1] = Phi_approx(mu_ndt[1] + sigma_ndt[1] * ndt_pr[i,1]) * min(RT_min[i,,1]);
    // Non-decision time time 2
    ndt[i,2] = Phi_approx(mu_ndt[2] + sigma_ndt[2] * ndt_pr[i,2]) * min(RT_min[i,,2]);
  }
}
model {
  ...
  to_vector(ndt_pr) ~ normal(0, 1); 
  
  // For each subject
  for (i in 1:N) {
    // Congruent at time 1
    for (t in 1:T_subj[i,1,1]) {
      RT[i,1,1,t] ~ shiftlnorm_lpdf(ndt[i,1], mu_i_base[i,1], sigma_i_base[i,1]);
    }
    // Incongruent at time 1
    for (t in 1:T_subj[i,2,1]) {
      RT[i,2,1,t] ~ shiftlnorm_lpdf(ndt[i,1], mu_i_base[i,1] + mu_i_delta[i,1], sigma_i_delta[i,1]);
    }
    // Congruent at time 2
    for (t in 1:T_subj[i,1,2]) {
      RT[i,1,2,t] ~ shiftlnorm_lpdf(ndt[i,2], mu_i_base[i,2], sigma_i_base[i,2]);
    }
    // Incongruent at time 2
    for (t in 1:T_subj[i,2,2]) {
      RT[i,2,2,t] ~ shiftlnorm_lpdf(ndt[i,2], mu_i_base[i,2] + mu_i_delta[i,2], sigma_i_delta[i,2]);
    }
  }
}
generated quantities { 
  ...
  for (i in 1:N) {
    // Congruent at time 1
    for (t in 1:T_subj[i,1,1]) {
      post_pred_c1_t1[i,t] = shiftlnorm_rng(ndt[i,1], mu_i_base[i,1], sigma_i_base[i,1]);
      log_lik[i,1,1,t] = shiftlnorm_lpdf(RT[i,1,1,t] | ndt[i,1], mu_i_base[i,1], sigma_i_base[i,1]);
    }
    // Incongruent at time 1
    for (t in 1:T_subj[i,2,1]) {
      post_pred_c2_t1[i,t] = shiftlnorm_rng(ndt[i,1], mu_i_base[i,1] + mu_i_delta[i,1], sigma_i_delta[i,1]);
      log_lik[i,2,1,t] = shiftlnorm_lpdf(RT[i,2,1,t] | ndt[i,1], mu_i_base[i,1] + mu_i_delta[i,1], sigma_i_delta[i,1]);
    }
    // Congruent at time 2
    for (t in 1:T_subj[i,1,2]) {
      post_pred_c1_t2[i,t] = shiftlnorm_rng(ndt[i,2], mu_i_base[i,2], sigma_i_base[i,2]);
      log_lik[i,1,2,t] = shiftlnorm_lpdf(RT[i,1,2,t] | ndt[i,2], mu_i_base[i,2], sigma_i_base[i,2]);
    }
    // Incongruent at time 2
    for (t in 1:T_subj[i,2,2]) {
      post_pred_c2_t2[i,t] = shiftlnorm_rng(ndt[i,2], mu_i_base[i,2] + mu_i_delta[i,2], sigma_i_delta[i,2]);
      log_lik[i,2,2,t] = shiftlnorm_lpdf(RT[i,2,2,t] | ndt[i,2], mu_i_base[i,2] + mu_i_delta[i,2], sigma_i_delta[i,2]);
    }
  }
} 
