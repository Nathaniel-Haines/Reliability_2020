functions {
  ...
  // with added a mixture component to deal with "contamination" responses
  real shiftlnorm_mix_lpdf(real x, real delta, real mu, real sigma, 
                           real lambda, real min_RT, real max_RT) {
    real log_pr;
    // if the RT is less than non-decision time, only return log_lik for uniform
    if (x < delta) {
      return(log(lambda) + uniform_lpdf(x | min_RT, max_RT));
    // else, return log_lik for mixture
    } else {
      // log_lik for shifted lognormal distribution
      // log_pr = (-log((x - delta)*sigma*sqrt(2*pi())) - (log(x - delta) - mu)^2 / (2*sigma^2));
      // return log_mix(lambda, uniform_lpdf(x | min_RT, max_RT), log_pr);
      return log_sum_exp(
        log(lambda) + uniform_lpdf(x | min_RT, max_RT), 
        log1m(lambda) + lognormal_lpdf(x - delta | mu, sigma)
      );
    }
  }
  // for generating posterior predictions
  real shiftlnorm_mix_rng(real delta, real mu, real sigma,
                          real lambda, real min_RT, real max_RT) {
    int I_unif; 
    // determine whether contamination or process RT
    I_unif = bernoulli_rng(lambda);
    if (I_unif==1) {
      return uniform_rng(min_RT, max_RT); // contamination
    } else {
      return delta + lognormal_rng(mu, sigma); // shifted lognormal process
    }
  }
}
data {
	...
	real RT_max[N, N_cond, N_time]; // Maximum RTs for each subject, condition, and timepoint
}
transformed data {
  vector[N] RT_min_all;
  vector[N] RT_max_all;
  for (i in 1:N) {
    RT_min_all[i] = 0;
    RT_max_all[i] = max(to_matrix(RT_max[i,:,:]));
  }
}
parameters {
...
transformed parameters {
  ...
  matrix<lower=0>[N,2] ndt_i;
  vector<lower=0,upper=1>[N] lambda_i;
  ...
  for (i in 1:N) {
    ...
    ndt_i[i,1] = exp(ndt_mean[1] + ndt_sd[1] * ndt_i_pr[i,1]);
    // Non-decision time time 2
    ndt_i[i,2] = exp(ndt_mean[2] + ndt_sd[2] * ndt_i_pr[i,2]);
    
    // mixing probability for contamination RTs
    lambda_i[i] = Phi_approx(lambda_mean + lambda_sd * lambda_i_pr[i]);
  }
}
model {
  ...
  mu_mean_base     ~ normal(-.5, .5);
  mu_mean_delta    ~ normal(0, 1);
  sigma_mean_base  ~ normal(-1, .5);
  sigma_mean_delta ~ normal(0, 1); 
  ndt_mean         ~ normal(-1.5, .3);
  lambda_mean      ~ normal(-1.65, .1);
  
  // Priors on group-level SDs
  mu_sd_base     ~ normal(0, .5);
  mu_sd_delta    ~ normal(0, .5);
  sigma_sd_base  ~ normal(0, .5);
  sigma_sd_delta ~ normal(0, .5);
  ndt_sd         ~ normal(0, .5);
  lambda_sd      ~ normal(0, .2);
  ...
  lambda_i_pr                 ~ normal(0, 1); 
  
  // For each subject
  for (i in 1:N) {
    // Congruent at time 1
    for (t in 1:T_subj[i,1,1]) {
      RT[i,1,1,t] ~ shiftlnorm_mix(
        ndt_i[i,1], 
        mu_i_base[i,1], 
...
generated quantities { 
  ...
  for (i in 1:N) {
    // Congruent at time 1
    for (t in 1:T_subj[i,1,1]) {
      post_pred_c1_t1[i,t] = shiftlnorm_mix_rng(
        ndt_i[i,1], 
        mu_i_base[i,1], 
        exp(sigma_i_base[i,1]),
        lambda_i[i],
      ...
      log_lik[i,1,1,t] = shiftlnorm_mix_lpdf(
        RT[i,1,1,t] | 
        ndt_i[i,1], 
        mu_i_base[i,1], 
        exp(sigma_i_base[i,1]),
    ...
    // Incongruent at time 1
    for (t in 1:T_subj[i,2,1]) {
      post_pred_c2_t1[i,t] = shiftlnorm_mix_rng(
        ndt_i[i,1], 
        mu_i_base[i,1] + mu_i_delta[i,1], 
        exp(sigma_i_base[i,1] + sigma_i_delta[i,1]),
        lambda_i[i],
      ...
      log_lik[i,2,1,t] = shiftlnorm_mix_lpdf(
        RT[i,2,1,t] | 
        ndt_i[i,1], 
        mu_i_base[i,1] + mu_i_delta[i,1],
        exp(sigma_i_base[i,1] + sigma_i_delta[i,1]),
    ...
    // Congruent at time 2
    for (t in 1:T_subj[i,1,2]) {
      post_pred_c1_t2[i,t] = shiftlnorm_mix_rng(
        ndt_i[i,2], 
        mu_i_base[i,2], 
        exp(sigma_i_base[i,2]),
        lambda_i[i],
      ...
      log_lik[i,1,2,t] = shiftlnorm_mix_lpdf(
        RT[i,1,2,t] | 
        ndt_i[i,2], 
        mu_i_base[i,2], 
        exp(sigma_i_base[i,2]),
    ...
    // Incongruent at time 2
    for (t in 1:T_subj[i,2,2]) {
      post_pred_c2_t2[i,t] = shiftlnorm_mix_rng(
        ndt_i[i,2], 
        mu_i_base[i,2] + mu_i_delta[i,2], 
        exp(sigma_i_base[i,2] + sigma_i_delta[i,2]),
        lambda_i[i],
      ...
      log_lik[i,2,2,t] = shiftlnorm_mix_lpdf(
        RT[i,2,2,t] | 
        ndt_i[i,2], 
        mu_i_base[i,2] + mu_i_delta[i,2],
        exp(sigma_i_base[i,2] + sigma_i_delta[i,2]),
