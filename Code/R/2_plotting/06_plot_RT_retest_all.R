...
results <- foreach(d=data_names, .combine = "rbind") %do% {
  ...
  tmp <- foreach(i=fit_names, .combine = "rbind") %do% {
    ...
    pars <- rstan::extract(readRDS(paste0("Data/2_Fitted/fit_", d, "_", i, ".rds")),
                           pars = c("R_mu", "R_sigma"))
    ...
    data.frame(Model     = i,
               ...
               mean      = c(mean(pars$R_mu[,1,2]), 
                             mean(pars$R_sigma[,1,2])),
               lb        = c(HDIofMCMC(pars$R_mu[,1,2])[1], 
                             HDIofMCMC(pars$R_sigma[,1,2])[1]),
               ub        = c(HDIofMCMC(pars$R_mu[,1,2])[2], 
                             HDIofMCMC(pars$R_sigma[,1,2])[2]))
  }
...
# Tor delay discounting data
fit_ddt <- readRDS("Data/2_Fitted/fit_Study1-DDT_hyperbolic.rds")
pars_ddt <- rstan::extract(fit_ddt)
...
# Store in long formatted data.frame
results_ddt <- data.frame(Model     = c(rep("Two-Stage MLE\nHyperbolic Model", 2), 
                                        rep("Full Generative\nHyperbolic Model", 2)),
                          Task      = rep("Delay Discounting", 4), 
                          Parameter = rep(c("log(k)", "log(c)"), 2),
                          mean      = c(k_cor$estimate, 
                                        c_cor$estimate,
                                        mean(pars_ddt$R_k[,1,2]), 
                                        mean(pars_ddt$R_c[,1,2])),
                          lb        = c(k_cor$conf.int[1],
                                        c_cor$conf.int[1],
                                        HDIofMCMC(pars_ddt$R_k[,1,2])[1], 
                                        HDIofMCMC(pars_ddt$R_c[,1,2])[1]),
                          ub        = c(k_cor$conf.int[2],
                                        c_cor$conf.int[2],
                                        HDIofMCMC(pars_ddt$R_k[,1,2])[2], 
                                        HDIofMCMC(pars_ddt$R_c[,1,2])[2])) 
rm(fit_ddt); gc()
...
p2 <- results_ddt %>%
  mutate(Model = factor(Model, 
                        levels = (c("Two-Stage MLE\nHyperbolic Model",
                                    "Full Generative\nHyperbolic Model")),
                        labels = (c("Two-Stage MLE\nHyperbolic Model",
                                    "Full Generative\nHyperbolic Model")))) %>%
  ggplot(aes(y = Task, x = mean, xmin = lb, xmax = ub, color = Model)) +
  geom_pointinterval(position = position_dodge(width = .3), size = 2) +
  geom_vline(xintercept = -1, color = "darkgray") +
...
p3 <- (p1 / p2) + plot_layout(heights=c(7,1))

ggsave(p3, filename = "Data/3_Plotted/test_retest_all.pdf", unit = "in",
       width = 8, height = 6)