...
results <- foreach(d=data_names, .combine = "rbind") %do% {
  ...
  tmp <- foreach(i=fit_names, .combine = "rbind") %do% {
    ...
    # test-retest correlations for mu and sigma
    R_mu_delta <- foreach(s=1:samps, .combine = "c") %do% {
      cor(pars$mu_i[s,,2] - pars$mu_i[s,,1], 
          pars$mu_i[s,,4] - pars$mu_i[s,,3])
    }
    R_sigma_delta <- foreach(s=1:samps, .combine = "c") %do% {
      cor(log(pars$sigma_i[s,,2]) - log(pars$sigma_i[s,,1]), 
          log(pars$sigma_i[s,,4]) - log(pars$sigma_i[s,,3]))
    }
    # Compute posterior means and 95% HDIs for both mu and sigma
    data.frame(Model     = i,
               Task      = d,
               Parameter = c("mu", "sigma"),
               mean      = c(mean(R_mu_delta), 
                             mean(R_sigma_delta)),
               lb        = c(HDIofMCMC(R_mu_delta)[1], 
                             HDIofMCMC(R_sigma_delta)[1]),
               ub        = c(HDIofMCMC(R_mu_delta)[2], 
                             HDIofMCMC(R_sigma_delta)[2]))
  }
...
ggsave(p1, filename = "Data/3_Plotted/test_retest_all_jointSingle.pdf", unit = "in",
       width = 8, height = 5)
