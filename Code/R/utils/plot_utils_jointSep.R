...
plot_retest <- function(pars, parameter, task, samp_data, legend) {
  ...
  est1 <- switch(parameter,
                 "mu" = fits[[1]]$R_mu_delta[,1,2],
                 "sig" = fits[[1]]$R_sigma_delta[,1,2])
  est2 <- switch(parameter,
                 "mu" = fits[[2]]$R_mu_delta[,1,2],
                 "sig" = fits[[2]]$R_sigma_delta[,1,2])
  est3 <- switch(parameter,
                 "mu" = fits[[3]]$R_mu_delta[,1,2],
                 "sig" = fits[[3]]$R_sigma_delta[,1,2])
  qplot() +
